...
@blueprint.route('/product/<slug>', methods=['GET', 'POST'])
def product_details(slug):
    ...
    if request.method == 'POST':
        ...
        order = OrderClient.add_to_cart(product_id=product['id'], quantity=1)
        session['order'] = order['result']
        flash("Product added to the cart")

    return render_template('product_info.html', product=product, form=form)
...
def checkout():
    ...
    if 'order' not in session:
        flash("Please add some products to the cart")
        return redirect(url_for("frontend.index"))
